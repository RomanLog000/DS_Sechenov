...
print(f"Avg width: {np.mean(widths):.1f}, Avg height: {np.mean(heights):.1f}")
print(f"Size variability: {np.std(widths):.1f} x {np.std(heights):.1f}")
...
> 1. As part of this graduation project, an automated classification system for skin lesion images based on the ISIC-2017 dataset was implemented. The project included a complete workflow — from exploratory data analysis and normalization to data augmentation and class distribution analysis. Both classical convolutional architectures (ResNet18, EfficientNet-B0) and a transformer-based model (ViT) were trained and compared. Among them, the ViT (Vision Transformer) demonstrated superior performance across all major metrics.

> 2. The ViT model fine-tuned with class balancing showed strong generalization ability on validation and test subsets — achieving ≈0.9 accuracy on validation and ≈0.6 on test data. This indicates that the approach is well-suited for practical applications under data-limited conditions. The study focused on binary classification (melanoma vs. seborrheic keratosis) — a clinically relevant problem of distinguishing malignant and benign morphologically similar lesions. Benign nevi were intentionally excluded to reduce class imbalance and enhance diagnostic clarity.

> 3. The resulting pipeline includes comprehensive preprocessing, augmentation, and multi-architecture model training. The balanced ViT model achieved the best balance between sensitivity and specificity, substantially improving melanoma recall on independent test data. While CNN baselines failed to generalize well given dataset size and variability, the transformer-based approach demonstrated clear advantages and high potential for further clinical adaptation.
...
# 1. Load test data
test_csv = "/content/drive/MyDrive/Colab Notebooks/Data/ISIC-2017_Test_v2_Part3_GroundTruth.csv"
test_dir = "/content/drive/MyDrive/Colab Notebooks/Data/ISIC-2017_Test_v2_Data/"

df_test = pd.read_csv(test_csv)
# Keep only melanoma and SK
df_test = df_test[df_test[["melanoma", "seborrheic_keratosis"]].sum(axis=1) > 0]
df_test["class"] = df_test.apply(
    lambda x: "melanoma" if x["melanoma"] == 1 else "seborrheic_keratosis", axis=1
)
df_test["path"] = df_test["image_id"].apply(lambda x: f"{test_dir}{x}.jpg")

id2label = {0: "melanoma", 1: "seborrheic_keratosis"}
...
class ISICDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(row["path"]).convert("RGB")
        inputs = self.processor(images=image, return_tensors="pt")
        return {k: v.squeeze() for k, v in inputs.items()}, label2id[row["class"]]
...
model.load_state_dict(torch.load("best_vit_balanced.pth", map_location=device))
model.eval()

# 4. Inference loop
dataset = ISICDataset(df_test, processor)
...
# 5. Visualization of random predictions
plt.figure(figsize=(15, 6))
samples = random.sample(range(len(df_test)), 5)
for i, idx in enumerate(samples):
    image = Image.open(df_test.iloc[idx]["path"]).convert("RGB")
    true_label = df_test.iloc[idx]["class"]
...
plt.suptitle("Inference on test samples (Balanced ViT)", fontsize=14)
plt.show()

# 6. Build and export results table
df_test["pred_class"] = [id2label[p] for p in y_pred]
df_test["prob_SK"] = y_probs

output_path = "/content/drive/MyDrive/Colab Notebooks/Data/vit_inference_results.csv"
df_test.to_csv(output_path, index=False, encoding="utf-8")

print(f"Inference completed. Results saved to:\n{output_path}")
print("\nPreview:")
print(df_test[["image_id", "class", "pred_class", "prob_SK"]].head())

"""The inference visualization confirms that the fine-tuned ViT model is able to correctly identify most seborrheic keratoses with high confidence, while melanoma predictions remain more uncertain — reflecting the clinical difficulty of distinguishing visually similar lesions."""
